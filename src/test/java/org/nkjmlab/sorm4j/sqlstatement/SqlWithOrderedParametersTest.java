...
class SqlWithOrderedParametersTest {
  ...
  void testToSqlStatementStringObjectArray() {
    SqlStatement sqlSt =
        SqlWithOrderedParameters.toSqlStatement("select * from a where in(?)", List.of("alice", 1));
  ...
  void testToSqlStatementStringObject() {
    SqlStatement sqlSt =
        SqlWithOrderedParameters.toSqlStatement("select * from a where id=? and id=?", 1, 2);
