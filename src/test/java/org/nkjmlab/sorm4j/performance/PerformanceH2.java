...
public class PerformanceH2 {
  ...
  private final int batchInserts = 10;
  private final int reads = 10;
  private final int readLists = 1;
  ...
  public PerformanceH2(String mode) {
    ...
    this.connectionHelper =
        new DataSourceHelper(getPackageNameToPath() + "h2." + mode + ".h2.properties",
  ...
  private void dropAndCreateTable() {
    try (Connection connection = connectionHelper.getConnection()) {
      OrmMapper orm = Sorm.getOrmConnection(connection);
  ...
  private void execOrmMultiRowInsert(AllTypes[] as) {
    try (Connection connection = connectionHelper.getConnection()) {
      Sorm.getOrmConnection(connection).insert(as);
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
  ...
  private void execOrmSingleInsert(AllTypes a) {
    try (Connection connection = connectionHelper.getConnection()) {
      Sorm.getOrmConnection(connection).insert(a);
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
  ...
  private void profileReadSingleByPrimaryKey() {
    try (Connection connection = connectionHelper.getConnection()) {
      OrmMapper persist = Sorm.getOrmConnection(connection);
