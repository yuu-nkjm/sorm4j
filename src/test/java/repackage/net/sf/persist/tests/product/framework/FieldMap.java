...
public class FieldMap {
   ...
   * not allow queries on blobs
   */
  private boolean supportsQueryByValue = true;

  /**
   * Some types, such as MySQL year2 and year4 are handled using short's, but when read as a map
   * they are returned as java.sql.Date objects. It's quite hard to perform automatic conversions
   * among these types, therefore it's advisable to turn map comparison off for them and use manual
   * testing.
   */
  ...
  public FieldMap setSize(int size) {
    this.size = size;
    return this;
  }

  public boolean isSupportsQueryByValue() {
    return supportsQueryByValue;
  }
  ...
  public boolean isSupportsCompareMapValue() {
    return supportsCompareMapValue;
  }
