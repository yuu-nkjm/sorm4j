...
public final class TableMapping<T> extends Mapping<T> {
  ...
  private int executeUpdate(Connection connection, String sql, final Object... parameters) {
    ...
    int ret = OrmConnectionImpl.executeUpdateAndClose(options, connection, sqlParametersSetter, sql,
        parameters);
    dp.ifPresent(lp -> {
      ...
      lp.debug(TableMapping.class, "{} Call executeUpdate for [{}] to [{}] Table at [{}]",
          lp.getTagAndElapsedTime(), getObjectClass().getSimpleName(), tableMetaData.getTableName(),
  ...
  public InsertResult<T> insertAndGet(Connection connection, T object) {
    ...
    try (PreparedStatement stmt = (!tableMetaData.hasAutoGeneratedColumns())
      ...
      sqlParametersSetter.setParameters(options, stmt, getInsertParameters(object));
      ...
      dp.ifPresent(lp -> {
        ...
        lp.debug(TableMapping.class, "{} Call executeUpdate for [{}] to [{}] Table at [{}]",
            lp.getTagAndElapsedTime(), getObjectClass().getSimpleName(),
            tableMetaData.getTableName(), Try.getOrNull(() -> connection.getMetaData().getURL()),
            insertSql);
      });
