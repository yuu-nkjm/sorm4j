...
import static org.nkjmlab.sorm4j.jmh.kitchensink.OrmBenchmark4.Post.*;
...
public class OrmBenchmark4 {
  ...
  private static final int NUM_OF_ROWS = 1024;

  public static void main(String[] args) {

    OrmBenchmark4 b = new OrmBenchmark4();
    b.setup();
    b.handCodedSelect();
  ...
  public void setup() {
    ...
    sorm.accept(conn -> {
      ...
      conn.executeUpdate("CREATE INDEX id_post_index ON post(id)");
    });
  }

  @Benchmark
  public void handCodedSelect() {
    handCodedSelect.selectSingleRowTypically(idGenerator());
  }

  @Benchmark
  public void sql2oTypicalSelect() {
    sql2oSelect.selectSingleRowTypically(idGenerator());
  }
  ...
  public void jDBISelect() {
    jdbiSelect.selectSingleRowTypically(idGenerator());
  }

  @Benchmark
  public void jOOQSelect() {
    jooqSelect.selectSingleRowTypically(idGenerator());
  }

  @Benchmark
  public void apacheDbUtilsSelect() {
    apacheDbUtilsSelect.selectSingleRowTypically(idGenerator());
  }

  @Benchmark
  public void myBatisSelect() {
    myBatisSelect.selectSingleRowTypically(idGenerator());
  }

  @Benchmark
  public void springJdbcTemplateSelect() {
    springJdbcTemplateSelect.selectSingleRowTypically(idGenerator());
  }

  @Benchmark
  public void sormSelect() {
    sormSelect.selectSingleRowTypically(idGenerator());
  }
  ...
  static class Sql2oSelect implements BenchmarkBase {
    ...
    @Override
    public void selectSingleRowTypically(int input) {
      try (org.sql2o.Connection conn = sql2o.open()) {
        Query query = conn.createQuery(Post.SELECT_TYPICAL_SQL + " WHERE id = :id")
    ...
    public void selectSingleRowWithOptimally(int input) {
      try (org.sql2o.Connection conn = sql2o.open()) {
        Query query = conn.createQuery(Post.SELECT_OPTIMAL_SQL + " WHERE id = :id");
        query.addParameter("id", input).executeAndFetchFirst(Post.class);
      }
    }
  ...
  static class JdbiSelect implements BenchmarkBase {
    ...
    @Override
    public void selectSingleRowTypically(int input) {
      jdbi.withHandle(handler -> handler.createQuery(Post.SELECT_TYPICAL_SQL + " WHERE id=:id")
          .bind("id", input).mapToBean(Post.class).findFirst()).get();
  ...
  public static interface BenchmarkBase {
    void selectSingleRowTypically(int input);
  ...
  public static class Post {
    public static final String DROP_AND_CREATE_TABLE = "DROP TABLE IF EXISTS post;"
        + "CREATE TABLE post ( "
        + "autoid INT NOT NULL IDENTITY PRIMARY KEY , text VARCHAR(255) , creation_date DATETIME , last_change_date DATETIME , "
        + "counter1 INT , counter2 INT , counter3 INT , counter4 INT , counter5 INT , counter6 INT , counter7 INT , counter8 INT , id INT );";
    public static final String SELECT_TYPICAL_SQL = "SELECT * FROM post";
    public static final String SELECT_OPTIMAL_SQL =
        "SELECT autoid, text, creation_date as creationDate, last_change_date as lastChangeDate, counter1, counter2, counter3, counter4, counter5, counter6, counter7, counter8, id FROM post";
    ...
    // name is id but not primary key
    private Integer id;


    public static Post createRandom(int id) {
